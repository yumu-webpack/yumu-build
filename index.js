...
var options = [
  ['-V', '--version', 'The version of yumu-build'],
  ['-h', '--help', 'The help of yumu-build']
]
...
function outputHelpInfo(options) {
  console.log(chalk.yellow('  Usage: build [option] <type>'));
  console.log('');
...
function init() {
  ...
  var getResult;
  try {
    getResult = child_process.execSync('npm run build', { encoding: 'utf8' });
  } catch(err) {
